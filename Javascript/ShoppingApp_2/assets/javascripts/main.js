...
function searchProduct() {
    ...
    if (value.length > 2) {
        for (var i = 0; i < products.products.length; i++) {
            if (products.products[i].p_name.includes(value) || products.products[i].p_brand.includes(value) || products.products[i].p_category.includes(value)) {
                // console.log(products[i].p_name);
                var li = document.createElement("li");
                li.innerHTML = products.products[i].p_name;
                ul.appendChild(li);
